...
## v5.3.0（对话隔离 + reset 语义调整）— 切卡不再串戏 + reset 保留记忆

两条用户侧行为变更，其中第二条**改变了 `/quill reset` 的既有语义**。

**1. 切换角色卡自动隔离对话历史（新能力）**

- 问题：AstrBot 的 conversation 只按 UMO 切分，**不按角色卡切分**。
  `memories` / `chat_logs` 早已按 `UMO::persona` 隔离，唯独 AstrBot 侧那段
  对话历史（即 `req.contexts`）没有隔离 —— 所以换角色卡后，新角色仍能读到
  上一个角色的对话，还会顺着旧上下文继续演。
- 修复：给每张角色卡绑定它自己的 AstrBot 对话（`conversation_manager`），
  切卡时自动切过去。切回来仍能看到该角色卡原来那段历史，是**隔离而非清空**。
- 时序关键：切换挂在 `on_waiting_llm_request`，该事件早于 AstrBot 的
  `_get_session_conv()`（`internal.py:225` vs `239`），因此**本轮立即生效**；
  若挂在 `on_llm_request` 则要等下一轮，会出现「切完第一句还带着旧上下文」。
- 迁移行为：首次启用时，**当前已绑定的角色卡接管现有对话**（历史不断），
  之后切到别的卡才为它新建空对话。
- 健壮性：拿不到 `conversation_manager`、或绑定的对话被 Dashboard 删除时
  都能自愈（重建而不是每轮空切），异常一律降级放行，不影响正常聊天。
- 新增 `UserState.persona_convs` 字段（旧状态文件兼容，缺失键走默认值）。

**2. `/quill reset` 不再清除记忆（破坏性语义变更）**

- 旧行为：删除该会话**所有角色卡**的动态记忆 + 对话日志，计数器归零。
- 新行为：**保留动态记忆**（长期记忆跨重置保留，AI 仍记得设定与过往），
  只清「当前角色卡」的对话上下文 + 对话日志，计数器归零。
- 即：`/quill reset` 现在是「重开本角色卡这段剧情」，而不是「抹掉记忆」。
  要彻底清记忆请用 `/memory clear`。
- 同时修正作用域：此前是 `delete_all_session_memories/chat_logs`（清所有角色卡），
  与「按卡隔离」的设计相悖；现在只清当前卡，不影响其他卡。
- 文案同步更新：`/char` 切换提示、`/quill help` 速查表、reset 回复。

**其他**
- `commands.py`：移除 `quill_reset` 中取出后从未使用的 `persona_id` 死变量。
