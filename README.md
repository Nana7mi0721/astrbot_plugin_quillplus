...
- **Fail-close 权限**：admin_users 未配置时，群聊写指令与面板编辑功能自动锁定（返回 503），杜绝公网裸奔
...
> ⚠️ **升级须知（v5.2 Breaking Change）**
>
> v5.2 起 `admin_users` 采用 fail-close 策略：**留空时群聊写指令（切卡/清记忆/改流式等）与面板编辑功能将全部锁定（返回 503）**。升级后请务必在 `[权限]` 分组填写管理员 ID，并在面板"安全与权限控制"卡片中设置本机管理员标识。
>
> 私聊不受此限制，所有写指令在私聊中仍可正常使用。

**推荐配置**：
- **首次使用**：务必先在 `[权限]` 填写 admin_users，否则群聊与面板写功能不可用
...
**Q: 升级后群聊指令/面板编辑不能用了？**
A: v5.2 起 admin_users 采用 fail-close 策略，留空时写功能锁定。请在 `[权限]` 配置管理员 ID，并在面板"安全与权限控制"卡片设置本机管理员标识。私聊不受影响。
...
- 🎨 **管理面板 UI 全面升级**
- 🛡️ **安全审计与并发加固**（fail-close 权限、XSS 防御、原子写入、FAISS 一致性、TOCTOU 修复、30+ 问题修复）
