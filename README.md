...
- **窗口架构（边栏优先）**：侧栏是应用骨架，承载分组导航与六个页面的实时计数（可折叠）；内容区为通栏标题带 + 列表，页面操作收进主按钮与 `⋯` 溢出菜单
- **控件体系**：建立控件高度令牌（`--h-control` 等）与 macOS 字阶，所有并排控件高度统一，消除基线错位
- **macOS 桌面交互**：弹层从窗口顶部垂落（sheet 语义）、右键上下文菜单、列表方向键导航（roving tabindex）、双击直接编辑、行内操作悬停显现
- **自绘下拉菜单**：11 处下拉全部替换为自绘 listbox——材质浮层、选中项打勾、方向键/首字母跳转、视口边缘自动翻转；原生 `<select>` 保留为值存储，功能零回归
